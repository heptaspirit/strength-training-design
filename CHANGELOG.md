...
### feat: 动作库可扩展（别名归一 + 免改脚本新增动作 + 缺档案显式提示）

起因：提问「输入了代码里没预先编入的动作会怎样，你会自己适应吗」。当时的答案是**二者都不**——动作名走严格白名单，未登记即硬失败；而且 `catalog` 只允许给**已登记**动作调参，**新增动作必须改脚本源码**，调用方也无法自适应。这是一条真实的可用性缺口，本次补上。

- **新增别名归一（`ALIASES` + `normalize_key`）**：`exercise` 字段现在接受 ①库内 key ②中文名（深蹲 / 相扑硬拉 / 窄距卧推 / 引体 / 实力推…）③常见英文写法（deadlift / bench press / pull up / RDL / close grip bench…）。匹配不上时用模糊匹配给出最接近的候选，**不猜**——排期结论依赖具体动作，取错动作比报错糟
- **`catalog` 允许新增动作（不必再改脚本）**：对未登记的 key 视为新增，必填 `m`、`td`，以及 `ref`（有外部负重）或 `eff`（自重）之一；可选 `zh` / `rest` / `wpr` / `chain` / `prime`。**`chain` 与 `prime` 必须成对给出**，否则协同比算不出来，脚本直接报错而不硬凑。对已登记 key 仍是调参（可只给部分字段），同一段配置两种用法并存
- **`get_spec` → `spec_of`**：统一走「内置库 + catalog 新增/覆盖 + one_rm 覆盖」三层；新增 `valid_keys` / `zh_of` / `muscle_zh` / `chain_profile` / `prime_of` 辅助函数，显示名逐级回退（中文名 → key），不再对 `CATALOG[key]["zh"]` 硬取
- **修一处潜伏的位置错配 bug**：`chain_analysis` 原先对无链条档案的动作 `continue`，返回的 `rows` 与 `blocks` **不等长**，而调用方按位置索引 `rows[i]`——一旦出现缺档案动作，**后面所有动作的协同比都会串位**。现改为返回与 `blocks` 等长的列表（缺档案处为 `None`），并额外返回 `untracked` 清单
- **缺档案显式提示，不静默降级**：A / Rf / 惩罚 / 崩坏风险照常算（只依赖 `m` / `ref` / `td`）；未计入「课末肌群等效组」且协同比为 `n/a` 的动作，会在报告里打出 `⚠ 无链条档案` 并写明补档案方式——避免拿被低估的等效组去和 MRV 比
- **校验友好化**：缺 `sets` / `reps` 由裸 `KeyError` 堆栈改为「<课名> 第 N 个动作 '<key>' 缺 <字段>」；`catalog` 新增缺字段时明确列出缺哪几个；`--list` 尾注说明三种写法与新增方式；JSON 输出新增 `config.custom_exercises` / `exercises[].zh` / `untracked_exercises`
- **`references/consultation/session-strain-modeling.md` 新增 §10「动作库：怎么写名字、怎么加动作」**（三种写法表、新增字段表、缺档案后果），原「来源」顺延为 §11
- **新增 `dev/tests/test_session_strain.py`（34 项）**：别名归一（含"错拼必须报错、不静默取近似动作"）、
  `catalog` 新增动作的四类必填校验、`chain`/`prime` 成对约束、**链条行与 blocks 位置对齐**（护住本轮修的串位 bug）、
  自重/负重互斥、来源标签只升不降、顺序不变性 <1% 分辨率、样例 A 值回归基线
- **顺手加固可导入 API**：`simulate` 原先用 `b["rest"]` / `b["weight"]` 硬取，手写 block（不经 `parse_session`）会 `KeyError`；
  改为 `.get()` 取缺省，函数签名声明「至少要有 exercise / sets / reps」
- **验证**：与工作区原型逐项比对 A / Rf / penalty **零偏差**（无回归）；中文名与别名、新增动作（含/不含链条档案）、五类错误路径全部实测通过；技能自检链接 ✓、**pytest 52 passed**
- **SKILL.md**：版本 0.9.17 → 0.9.18

## [0.9.17] - 2026-09-15

### feat: 单次训练应激模型（session_strain 三轴 + sRPE 标定）

起因：用户在 C2/C3 实测中发现「8 组容量深蹲后人很累，面拉没问题但引体上气不接下气」「硬拉日同样，卧推日轻松」，并预判 C3 周一「深蹲+引体+实力推」必有一项做不动。MRV 审计对这一类问题完全静默——它只回答肌群组数。

- **新增 `scripts/session_strain.py`（三轴单次课审计）**：吃 session JSON（动作/组/次/重量），输出
  **A** 系统应激总量（两室模型：快恢复 F + 慢累积 S，组间双指数衰减，动作代价随累积量饱和放大）、
  **Rf 峰值**瞬时应激（"最喘的那一刻"）、
  **每动作 penalty / 崩坏风险**（技术需求 × 执行折扣）、
  **局部协同链协同比**（目标动作开始时最吃的那块协同肌 vs 主动肌的预疲劳深度）。
  内置参数为工程估算值（显式标注未标定），因此**只支持同文件内相对比较，不给绝对阈值**。
  支持 `--params` 覆盖参数 / `--anchor` 换刻度 / `--init-params PATH` 导出模板 / `--json` / `--list`。
  示例：`scripts/examples/session_strain_sample.json`（C3 三训练日）
- **新增 `references/consultation/session-strain-modeling.md`**：模型结构（两室 + 饱和放大 + 双指数恢复）、
  **四读法**（A 抓"人还剩多少" / Rf 抓峰值 / penalty 抓执行折扣 / 协同比抓链条预疲劳）、
  **三条参数无关结论**（① 顺序只转移代价不改变 session 总量；② 系统应激与神经强度轴在周期内负相关，
  故**不能合成单一数值**，合成会抵消信号；③ 肌群周组数守恒 ≠ 课内不被预疲劳）、
  排期决策顺序、已知局限（长而轻 session 高估、参数未标定时幅度不稳）
- **新增 `references/consultation/srpe-calibration.md`**：sRPE（session RPE，Foster 2001 CR10）
  测量协议、AU = sRPE × 时长、日志字段（**必须含"下一节计划完成度"**，否则阈值画不出来）、
  **三步标定升级路径**（估算 → 实测标定 → 用户直供长期参数）、文献边界
  （sRPE 在力量训练的已知缺陷：时长不是负荷的优良代理）
- **新增「模型分辨率」约定（`RESOLUTION_PCT = 1.0`）**：A 只读整数刻度，**差异 <1% 判为相等**，
  ≥1% 才当结论、≥10% 才当决策依据。依据：跨全部可能排列实测 A 极差 <0.7%（周三 5 动作 120 种 0.19%、
  深蹲日 7 动作 5040 种 0.70%），而参数不确定度是 ±100% 量级；
  **推论：「顺序不影响总量、只转移代价」在模型分辨率内严格成立**（输出表头已加分辨率提示行）
- **接线**：`workflows/design-plan.md` 步骤 5 容量审计改为 **MRV 审计 + 课内应激审计并列**（新增步骤 5b），
  新增审计超限处置行；`workflows/consult.md` 新增"单次课负荷/排期"路由行；
  `workflows/modify-plan.md` 新增"某天太累/某动作做不动/想换顺序"前置检查；
  `guardrails.md` 新增反例"只做 MRV 周组数审计就认定某训练日安全"；
  `SKILL.md` 硬规则与 references 清单同步登记
- **顺带修掉工作区原型的一处用例定义缺陷**：`session-strain/session_strain_v3.py` 的 `C3_OHP_FIRST`
  写成 `[ohp] + C3_MON[:-1]`，实际得到 **OHP 出现两次、RDL 丢失**的错误课表（A 虚高到 14.41）。
  已改为真正的重排（OHP 提到最前、其余顺序不变），修正后 A=14.24（+0.18%，在分辨率内）。
  `verify_vs_skill.py` 的顺序不变性段同步改为实测口径（不再手写"均 <0.3%"这类与实测不符的断言）
- **验证**：与工作区原型 `session-strain/` 逐项核对，A / Rf / 逐动作 penalty / 协同比**零偏差**；
  CLI 全路径（demo / list / json / params / 错误路径）跑通；
  `--init-params` 导出的模板经**原样回填自检**（修复了模板给出 `deadlift` 这类非动作库 key、回填即报错的缺陷，
  改为按动作库带参考 1RM 的条目生成，硬拉为 `dl_conv` / `dl_sumo`）
- **SKILL.md**：版本 0.9.16 → 0.9.17

## [0.9.16] - 2026-09-15

### feat: 单次剂量上限与容量分散（guardrails 第 7 条）

- **`guardrails.md` 新增第 7 条硬约束「单次剂量上限与容量分散」**：明确 **MRV 是「单次剂量 × 频率 × 恢复窗口」的函数，周总量达标 ≠ 安全**——同一肌群单次训练 ≤6-8 个有效组（受伤窗口开在过量后的那几组：疲劳掩盖下先崩的是肩袖 / 肩胛控制 / 腰椎这类稳定结构，而重量还在往上加）；周容量须分散到 ≥2 天、同模式两次间隔 ≥48h；高剂量日不排在全身性疲劳日（如硬拉次日）之后；新进动作 W1 从 2 组起步（爬坡）。反例表同步补 2 条（单日集中某肌群全部容量；新动作 W1 直接上常规组数）
- **`volume-recovery/mrv-audit.md` 新增「第一步补充：单次剂量与分散」**：审计流程改为**先按训练日查单次剂量 / 分散度 / 恢复窗口 / 爬坡，再比对周总量**，含 5 项检查表（含「同一训练日推举类合计 ≤8 组、肩部直接推举 ≤4 组」硬上限）与一个真实反例（肩部推举全堆周六 = 同日 10 组、紧接硬拉日、周容量 3 倍跳，周总量"刚好在 MRV 内"但实际高风险）；「常见错误」补第 7-8 条；修正「群体平均值」处括号不匹配的笔误
- **`guardrails.md` 硬约束 6**：去掉残留的硬编码周次「（W7 起）」，改为 phase 表述「冲刺（峰值）phase」
- **SKILL.md**：版本 0.9.15 → 0.9.16

## [0.9.15] - 2026-09-10

### fix+feat: 有氧心率处方链一致性修复（VT1 个体校准 + Tanaka 降级路径）

- **`output/output-templates.md` 有氧表修正（自相矛盾修复）**：原表用 `Zone 2 (60-70% HRmax)` 并给出 `Zone 2 = (220 - 年龄) × 60-70%` 示例——直接违反 guardrails「不得用 220 − 年龄」与 aerobic-training.md §1 的规定。改为 %HRR 表述（Zone 2 = 中等 40-59% HRR），换算指引改为 Karvonen + 实测 HRmax 优先 / Tanaka 降级，并新增「个体校准优先于公式（说话测试锚定 VT1）」条目
- **`exercises/aerobic-training.md` 补强**：§1.1 补 Tanaka 方程具体形式（HRmax = 208 − 0.7 × 年龄；Tanaka 2001 JACC，351 项研究 18,712 人 Meta，40 岁以上比 220−年龄系统误差更小），作为 ACSM「人群特异性方程」的落地；§1.3 补「公式之后做个体校准」——任何心率公式都是 VT1 的人群代理，用说话测试锚定 VT1 后以其心率作为恢复性有氧实际上限
- **`scripts/gpp_calculator.py` hr 子命令增强**：`--hr_max` 由必填改为可选，新增 `--age`（Tanaka 估算降级路径）；输出标注 HRmax 来源；尾注补充 Tanaka 优先级与说话测试锚定提示。Karvonen 公式与 ACSM 五档 %HRR 分级本身未动（与 §1.3 一致，核对无误）
- **SKILL.md**：版本 0.9.14 → 0.9.15

## [0.9.14] - 2026-09-09

### feat: 抗伸展核心训练正式化（Rollout 专项）

- **新增 `references/health/core-training.md`「抗伸展专项：Rollout」章节**：定位说明（深蹲/硬拉中腹肌 EMG 普遍 <25% MVIC，三大项不自动补前侧抗伸展；rollout 是少数可靠加长力臂渐进超载的腹部动作）、**六档变式谱系**（墙面 → 斜面 → 跪姿 → TRX 悬吊 fallout → Pike/Knee Tuck → 站姿）、进阶路径（力臂主线 + 角度支线）、晋级门槛（跪姿全程 3×8 无代偿才进站姿）、剂量（3×6-10、离心 2-3 秒、RPE 7-8、不力竭、每周 1-2 次）、时序与周期落位、风险与禁用
- **变式机制区分**：Pike / Knee Tuck 标注为**髋屈主导的动态屈曲**变式（股直肌激活显著更高），明确"不是更难的 rollout"，不混入抗伸展主进阶线；TRX fallout 标注其**无级调难度**特性，适配器械受限环境；杠铃 rollout 与腹轮等价，不作为设备门槛
- **分类表修正**：山羊挺身由「抗伸展」归位到**「后侧伸展」**（原归类有误），死虫式归位到抗伸展；新增「后侧伸展」一行；增补四类覆盖自检提示
- **周期落位更新**：容量期加入跪姿 rollout 打基础、力量期 3×6-10 @RPE 7-8 维持、冲刺期退回跪姿半程 2×8；补充后侧伸展的归口说明（山羊挺身/RDL 算后侧链日，核心模块不重复计）
- **参考文献补 5 条**：Escamilla 2006（Power Wheel EMG）、SBS Core Training Guide、Tayashiki 2016（8 周腹部支撑 → 深蹲峰值功率 +16%）、Saeterbakken 2022 meta、Steele 2015（每周 1 组即有效）
- **常见错误新增 Rollout 专项反例表** 6 条（排在主项后、做到力竭、跳级站姿、靠加次数制造难度、把 pike 当进阶、拿它替换 Pallof/农夫行走）
- **`workflows/design-plan.md` 步骤 4**：四项强制 → 五项，新增「核心四格自检」（抗伸展/抗旋转/抗侧屈/后侧伸展各占一格、四类不可互相替代）
- **`SKILL.md`**：版本 0.9.13 → 0.9.14，知识索引 health 行标注 Rollout 专项

## [0.9.13] - 2026-09-07

### feat: GPP 体能训练正式化（新增功能五 + 总纲 + 计算器）

- **新增 `references/methodology/gpp-framework.md`（GPP 整合总纲）**：五维分类（心肺/工作容量/局部耐力/活动度/NEAT）、三轴参数化（定位档×器械层×落位模式）、三档疲劳分类账（恢复性不计 / 维持性 5-10% / 发展性 15-25%）、周期阶段落位表、与主项同日时序规则、最小器械动作库、进阶与测试。**统一了此前 skill 内部三处互相冲突的「GPP 算不算恢复预算」口径**（aerobic 说不计、westside/gpp-recovery 说分性质计、concurrent 说会堆过 MRV）
- **重写 `references/exercises/aerobic-training.md`**：由 Zone 2 单一路线升级为完整心肺维度处方。修正三处硬伤——①「有氧促恢复」改为 NSCA 原意：对力量/爆发力项目**不必要甚至可能反效**，恢复靠减载与睡眠 ②「HIIT 绝对避免」改为有条件允许：须先有有氧基础、每周 ≤1 次、避开下肢离心模态 ③**「220 − 年龄」被 ACSM 12th 明确不推荐**（误差 10-15 bpm），改用 %HRR + Karvonen 公式。新增：FITT 剂量、强度分级多指标对照、五型有氧谱系、work-rest 表、间歇协议（含杠铃复合组）、心肺适能测试协议
- **更新 `references/methodology/concurrent-training-interference.md`**：以 Schumann 2022（43 项研究 / 1090 人）替换 Wilson 2012 + Hickson 1982 作为主要依据——**最大力量无显著干扰（SMD −0.06, p=0.446）、爆发力显著受干扰（SMD −0.28, p=0.007）且集中于有氧与抗阻同节完成**。保护重点由「最大力量」改到「爆发力」（对做高翻/高拉者尤为关键），缓解手段首推**分节/分天**而非单纯减量；补充剂量阈值（≤30 min 低强度不损害随后力量表现）与跑步 > 骑行/划船的模态差异
- **新增 `workflows/conditioning-plan.md`（功能五）**：GPP/体能计划 7 步工作流，含与功能三的边界判定表、硬约束 4 条、异常处理 7 项、输出格式 6 项
- **新增 `scripts/gpp_calculator.py`**：四子命令——`hr`（Karvonen 目标心率区间）、`workrest`（NSCA 间歇处方，含组数反推）、`budget`（GPP 恢复预算审计，含阶段违规与超限检测）、`progress`（NSCA 10% 进阶表）
- **`SKILL.md`**：能力矩阵新增功能五、硬约束 5 → 6 条（GPP 不反向改主项）、知识索引与参考文献更新（新增 Schumann 2022）、版本 0.9.12 → 0.9.13
- **`guardrails.md`**：新增第 6 条硬约束详述；反例表补 5 条 GPP 相关反例（全周期恒定有氧、发展性进冲刺期、砍主项给有氧让路、同节有氧在抗阻前、堆有氧促恢复）
- **`workflows/design-plan.md`**：步骤 4「核心稳定与有氧」→「GPP 与体能模块」，新增四项强制：定三轴 / 按阶段出方（禁止全周期恒定）/ %HRR 与 Karvonen 量化 / 疲劳口径走三档分类账
- **fix: 来源年份更正** — Westside *Book of Methods* 由 2007 更正为 2023，全库 18 处（脚本批量替换 + 残留验证为 0；RTS 的 Tuchscherer 2007 与 CHANGELOG 历史条目不受影响）
- **手段库按性质重分类**：`gpp-framework.md` 手段库拆为**代谢体能（A 组：战绳/药球砸/壶铃摆动/波比跳/农夫行走/划船机间歇，放主项课后）**与**增强式（C 组：跳箱/药球抛，属 SPP 爆发力、放主项课前，神经新鲜时做）**，明确两组时序相反；波比跳标注性价比低于战绳（脊柱反复屈伸、疲劳下动作质量崩坏）；T1 器械层补药球与跳箱；`conditioning-plan.md` 步骤 2/5 与异常处理表同步增强式归位说明
- **体例统一：外部文献引用降粒度** — 全库移除章节号/表号/图号/行号定位符（Ch5、Table 5.2、Box 5.1、L629 等），来源只保留到「书名 + 版本」粒度；内部文件的 § 导航引用保留不受影响
- **README/SKILL.md 措辞**：去掉刻意化表述，体系名称（JTS/RTS/Westside/Barbell Medicine/ACSM/NSCA）移至 README 末尾「方法论依据」单独一段

### Rationale

用户提出把体能训练在 skill 里正式化，并问是否需要引入 CrossFit 等外源资料。诊断发现 GPP 此前是碎片化的：心肺维度只有 Zone 2 单一路线、Westside 的工作容量条文挂在 `westside/` 分支下永不触发主线、且三份文件对「GPP 占不占恢复预算」口径不一致。通读用户已转 MD 的 NSCA 5th 与 ACSM 12th 后确认：**两份教科书已覆盖 GPP 心肺维度的全部骨架**（NSCA 的能量系统时间域与 work-rest 表、五型有氧谱系与进阶；ACSM 的 FITT 剂量、强度分级、间歇协议、测试），无需引入外源。同时发现现有文件存在路线级冲突（有氧促恢复、HIIT 绝对避免、220−年龄），故选择重写而非打补丁。证据层面按目前规模最大的 meta 更新干扰效应结论，并据此把 GPP 的设计重点从「保最大力量」转移到「保爆发力 + 分节」。
